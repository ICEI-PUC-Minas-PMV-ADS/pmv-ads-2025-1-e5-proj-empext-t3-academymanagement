...
import { useEstablishmentRepository } from '../../../../infrastructure/repositories/establishment';
...
export const EstablishmentGrid = () => {
	...
	const establishmentRepository = useEstablishmentRepository();

	const [tableData, setTableData] = useState<IEstablishmentEntitye[]>([]);

	const { filters, onFilters } = useFilters<IEstablishmentFilters>({
		initialFilters: {
			search: '',
		},
	...
	const queryDataTable = async () => {
		...
		try {
			const dataEstablishment =
				await establishmentRepository.getAll(debouncedSearch);

			if (dataEstablishment) setTableData(dataEstablishment || []);
			else
	...
	const queryDeleteEstablishment = async (
		...
		await itemsSelectedDeletion.reduce(async (previousPromise, id) => {
			...
			try {
				const data = await establishmentRepository.delete(id);
				results.push(data);
			} catch (error) {
				console.error(error);
	...
	const handleDeleteRow = useCallback(
		async (id: string) => {
			await queryDeleteEstablishment([id]);
	...
	const handleEditRow = useCallback(
		(id: string) => {
			router.push(Routes.establishmentEdit(id));
		},
		[router],
	);
